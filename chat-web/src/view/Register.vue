...
<div class="root" @mousemove="mv">
  ...
  <div class="brand">
    ...
    <div class="brand-desc">创建你的账号</div>
  </div>
  ...
  <div class="panel-area">
    <div class="panel">
      ...
      <el-form :model="f" ref="rf">
        <el-form-item prop="userName" class="fld">
          <el-input v-model="f.userName" autocomplete="off" placeholder="用户名" maxlength="20" @input="err.userName=''">
            <i slot="prefix" class="el-icon-user"></i>
          </el-input>
          <div class="fe" v-if="err.userName">{{err.userName}}</div>
        </el-form-item>
        <el-form-item prop="password" class="fld">
          <el-input type="password" v-model="f.password" autocomplete="off" placeholder="密码" maxlength="20" @input="err.password=''">
            <i slot="prefix" class="el-icon-lock"></i>
          </el-input>
          <div class="fe" v-if="err.password">{{err.password}}</div>
        </el-form-item>
        <el-form-item prop="confirmPassword" class="fld">
          <el-input type="password" v-model="f.confirmPassword" autocomplete="off" placeholder="确认密码" maxlength="20" @input="err.confirmPassword=''">
            <i slot="prefix" class="el-icon-lock"></i>
          </el-input>
          <div class="fe" v-if="err.confirmPassword">{{err.confirmPassword}}</div>
        </el-form-item>
        ...
        <el-button class="submit-btn" @click="register">注 册</el-button>
      </el-form>
      <router-link class="switch-link" to="/login">已有账号？<b>前往登录 →</b></router-link>
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  data() {
    return {
      mx: 0, my: 0,
      captchaImage: '',
      f: { userName: '', password: '', confirmPassword: '', captchaKey: '', captchaCode: '' },
      err: { userName: '', password: '', confirmPassword: '', captchaCode: '' }
    };
  },
  methods: {
    mv(e) {
    ...
    register() {
      ...
      if (!this.f.confirmPassword) { this.err.confirmPassword = '请确认密码'; ok = false; }
      else if (this.f.confirmPassword !== this.f.password) { this.err.confirmPassword = '两次密码输入不一致'; ok = false; }
      else { this.err.confirmPassword = ''; }
      if (!this.f.captchaCode) { this.err.captchaCode = '请输入验证码'; ok = false; } else { this.err.captchaCode = ''; }
      if (!ok) return;
      this.$http({ url: '/register', method: 'post', data: this.f })
        .then(() => this.$http({ url: '/login', method: 'post', data: { terminal: 1, userName: this.f.userName, password: this.f.password } }))
        .then(d => {
          saveLoginSession(d, { autoLogin: true, userName: this.f.userName });
          this.$message.success('注册成功');
          this.$router.push('/home/chat');
        });
    },
    // ========== Three.js 粒子背景（同 Login 保持一致） ==========
    init3(th) {
